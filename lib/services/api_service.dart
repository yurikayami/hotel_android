  ...
  void _handleError(http.Response response) {
    ...
    if (response.statusCode == 401) {
      ...
      try {
        ...
        final errorMsg = body['message'] ?? body['error'] ?? 'Client error';
        throw Exception(errorMsg);
      } catch (e) {
        throw Exception('Error: ${response.statusCode}');
      }
    } else if (response.statusCode >= 500) {
      print('[ApiService] Server error occurred');
      try {
        final body = jsonDecode(response.body);
        final errorMsg = body['message'] ?? 'Server error - Please try again later';
        print('[ApiService] Server error message: $errorMsg');
        throw Exception(errorMsg);
      } catch (e) {
